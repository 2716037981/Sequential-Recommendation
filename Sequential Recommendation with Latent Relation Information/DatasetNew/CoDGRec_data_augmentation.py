...
class Random(object):
    ...
    def __init__(self, tao=0.2, gamma=0.7, beta=0.2,
        ...
        # 长序列与短序列区分的阈值，长序列不进行mask,reorder操作，因为本身信息就不足
        ...
        if self.augment_threshold == -1:
            ...
            if self.augment_type_for_short == 0:  # SI
                print("use SI")
                self.long_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate,
                                                  ...
                                                  Substitute(item_similarity_model, substitute_rate=substitute_rate)]
            elif self.augment_type_for_short == 1:  # SIM
                print("use SIM")
                self.long_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate,
                                                         max_insert_num_per_pos=max_insert_num_per_pos,
                                                         augment_threshold=self.augment_threshold),
            ...
            elif self.augment_type_for_short == 2:  # SIR
                print("use SIR")
                self.long_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate,
                                                          max_insert_num_per_pos=max_insert_num_per_pos,
                                                          augment_threshold=self.augment_threshold),
                                                   Substitute(item_similarity_model, substitute_rate=substitute_rate),
                                                   Reorder(beta=gamma)]
            elif self.augment_type_for_short == 3:  # SIC
                print("use SIC")
                self.long_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate,
                                                         max_insert_num_per_pos=max_insert_num_per_pos,
                                                         augment_threshold=self.augment_threshold),
                                                  Substitute(item_similarity_model, substitute_rate=substitute_rate),
                                                  Crop(tao=tao)]
            elif self.augment_type_for_short == 4:   # SIMR
                print("use SIMR")
                self.long_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate,
                                                         max_insert_num_per_pos=max_insert_num_per_pos,
                                                         augment_threshold=self.augment_threshold),
            ...
            elif self.augment_type_for_short == 5:  # SIMC
                print("use SIMC")
                self.long_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate,
                                                         max_insert_num_per_pos=max_insert_num_per_pos,
                                                         augment_threshold=self.augment_threshold),
            ...
            elif self.augment_type_for_short == 6:  # SIRC
                print("use SIRC")
                self.long_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate,
                                                         max_insert_num_per_pos=max_insert_num_per_pos,
                                                         augment_threshold=self.augment_threshold),
            ...
            elif self.augment_type_for_short == 7:  # SIRCM
                print("use SIRCM")
                print("all aug set for long sequences")
                self.long_seq_data_aug_methods = [Insert(item_similarity_model, insert_rate=insert_rate,
                                                         max_insert_num_per_pos=max_insert_num_per_pos,
                                                         augment_threshold=self.augment_threshold),
                                                  Substitute(item_similarity_model, substitute_rate=substitute_rate),
            ...
            if index in insert_idx:
                ...
                if self.ensemble:
                    ...
                    inserted_sequence += self.item_similarity_model.most_similar(item,
                                                                                 top_k=top_k)
            inserted_sequence += [item]  # 这里插入时是将相似的物品插在原始物品前面
...
class Crop(object):
    ...
    # crop本质上就是随机截取用户序列的一部分返回,后续不足的部分依旧需要padding
...
class Mask(object):
    ...
    def __call__(self, sequence):
        ...
        for idx, mask_value in zip(mask_idx, mask):  # 这里mask_id与padding_id一致都是0

...
class MyTiSaSRecDataset(Dataset):
    def __init__(self, user_seq, item_num, max_seq_length, time_matrix_list, data_type='train'):
        self.user_seq = user_seq
        self.data_type = data_type
        self.max_len = max_seq_length
        self.item_num = item_num
        self.time_matrix_list = time_matrix_list
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == "train":
            ...
            answer = [0]  # no use
            cur_time_matrix = self.time_matrix_list[0][index]

        elif self.data_type == 'valid':
            input_ids = items[:-2]
            target_pos = items[1:-1]
            answer = [items[-2]]
            cur_time_matrix = self.time_matrix_list[1][index]

        else:
            input_ids = items[:-1]
            target_pos = items[1:]
            answer = [items[-1]]
            cur_time_matrix = self.time_matrix_list[2][index]
        ...
        cur_rec_tensors = (
            ...
            torch.tensor(cur_time_matrix, dtype=torch.long),
        )
...
def stamp_to_int(sorted_time_list):
    # 建立相对时间间隔字典
    time_min = int(1e12)          # must be python 3
    for tmp in sorted_time_list:
        for tt in tmp:
            time_min = min(time_min, tt)

    time_max = -1
    relative_time_seq = []
    for t_list in sorted_time_list:
...
# 获取训练集，验证集，测试集的time_matrix_list,list每个元素是numpy array数组
def get_time_matrix_list(relative_time_li,max_len,time_span,state="train"):
    time_matrix = []
    for time_list in relative_time_li:
        if state == "train":
            t_list = time_list[0:-3]
        if state == "valid":
            t_list = time_list[0:-2]
        if state == "test":
            t_list = time_list[0:-1]
        pad_len = max_len - len(t_list)
        input_ids = [0] * pad_len + t_list      # padding
        input_ids = input_ids[-max_len:]        # truncate
        time_matrix.append(computeRePos(input_ids,time_span))

    return time_matrix


def get_loader_graph_info_rating_matrix(data_path, batch_size, max_seq_len=50,time_span = 128,item_g=False, user_g=False):
    ...
    # 为训练集（不考虑后3个物品），验证集（不考虑后2个物品），测试集（不考虑最后一个物品）分别建立时间矩阵字典（key=用户id,value=(max_seq_len,max_seq_len)）
    relative_time_seq, time_min, time_max = stamp_to_int(sorted_time_list)
    time_matrix_info = [get_time_matrix_list(relative_time_seq, max_seq_len, time_span, state=t) for t in ["train", "valid", "test"]]
    print("数据集时间矩阵生成成功！")

    dataset_list = [MyTiSaSRecDataset(user_seq, item_num=para_dic['itemNum'], max_seq_length=max_seq_len,
                                      time_matrix_list=time_matrix_info,
                                      data_type=t)
    ...
    loader_list = []
    for i in range(3):
        loader_list.append(DataLoader(dataset=dataset_list[i], batch_size=256, sampler=sampler_list[i], num_workers=6,
                                      pin_memory=True))

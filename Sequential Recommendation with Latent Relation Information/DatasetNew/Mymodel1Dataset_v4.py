...
def preprocess_dataset_v1(dataset_list, timestamp_list, para_dic, args, item_shift=1):
    ...
    assert para_dic['userNum'] == len(user_seq)   # 用于判断一个表达式，在表达式条件为 false 的时候触发异常
...
def generate_rating_matrix_valid(user_seq, num_users, num_items):
    ...
    row = np.array(row) # 行指标
    col = np.array(col) # 列指标
    data = np.array(data) # 行列指标对应数据
    rating_matrix = csr_matrix((data, (row, col)), shape=(num_users, num_items)) # 按行压缩的办法, 将原始的矩阵用三个数组进行表示
...
class MySASRecDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        seq_set = set(items)
        # for循环中用下划线代替变量名
        # _作为一个标识符，当循环的值在下面不会用到，但又不知道起啥变量名的时候可以用它。
        for _ in input_ids:
...
if __name__ == "__main__":
    ...
    data_loader_list, item_g_info, _, rating_matrix, = get_loader_graph_info_rating_matrix(path, 128, max_seq_len=50,
                                                                                           item_g=True)
